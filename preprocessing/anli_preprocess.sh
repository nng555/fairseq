...
DATA_FOLDER="/scratch/ssd001/datasets/nli/anli"
...
for GENRE in $GENRES; do
  ...
  for AUG in $AUGS; do 
    if [[ "$AUG" != *"orig"* ]] && [[ "$AUG" != *"eda"* ]]; then
      ...
      cp $a_folder/orig/dev* $a_folder/$AUG
    ...
    for file in $FILES; do
      if [ ! -d "${DATA_FOLDER}/${GENRE}/${AUG}/bin/$file" ]; then
        fairseq-preprocess \
          ...
          --validpref "$DATA_FOLDER/$GENRE/$AUG/dev.$file" \
    ...
    if [ ! -d "${DATA_FOLDER}/${GENRE}/$AUG/bin/label" ]; then
      ...
      fairseq-preprocess \
        ...
        --validpref "$DATA_FOLDER/$GENRE/$AUG/dev.$dname.label" \
    ...
    cp $GLUE_DATA_FOLDER/$GENRE/orig/dev.soft.label $GLUE_DATA_FOLDER/$GENRE/$AUG/bin/label/valid.label
  done
done
