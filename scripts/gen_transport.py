...
def hf_reconstruction_prob_tok(masked_tokens, target_tokens, tokenizer, model, softmax_mask, reconstruct=False, return_probs=False, topk=1):
    single = False
...
def load_recon_model(recon, recon_date=None, recon_name=None, recon_rdset=None):
    if recon == "base" or not recon:
        ...
        raise Exception("reconstruction model %s not found".format(recon))
...
def gen_neighborhood(cfg: DictConfig):
    ...
    r_model, r_encode = load_recon_model(cfg.gen.recon, cfg.gen.recon_file.date, cfg.gen.recon_file.name, cfg.gen.recon_file.rdset)

    s_model, s_encode = load_recon_model(cfg.gen.comp, cfg.gen.comp_flie.date, cfg.gen.comp_file.name, cfg.gen.comp_file.rdset)
    ...
    extl_path = os.path.join(f_path, '.'.join([data_split, split, extl]))
    ...
    while (sents != []):
        ...
        for i in range(len(num_gen))[::-1]:
            if num_gen[i] == cfg.gen.num_samples or num_tries[i] > cfg.gen.max_tries:
                ...
                for sg, prob in zip(gen_sents[skip:], gen_probs[skip:]):
        ...
        if cfg.gen.recon in ['distil', 'german', 'multilingual']:
            probs = hf_reconstruction_prob_tok(toks, masks, tokenizer, r_model, softmax_mask, return_probs=True, topk=cfg.gen.topk)
        else:
            rec, rec_masks, rec_prob = r_model.reconstruction_prob_tok(toks, masks, return_probs=True, topk=cfg.gen.topk)
